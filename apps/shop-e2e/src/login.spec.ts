...
test.describe('Login Page', () => {
	...
	test('should show validation errors when fields are empty [same test with dataid]', async ({ page }) => {
		const loginButton = page.getByTestId("login-button") // this will get the job done
		await loginButton.click();
		// HTML5 validation check
		const emailInput = page.locator('#email');
		const isEmailValid = await emailInput.evaluate((el: HTMLInputElement) => el.checkValidity());
	...
	test('should login successfully with correct credentials', async ({ page }) => {
		...
		await expect(message).toHaveCSS('color', 'rgb(46, 125, 50)');
	});
